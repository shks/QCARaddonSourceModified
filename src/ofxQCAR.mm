...
class ofxQCAR_UpdateCallback : public UpdateCallback {
    virtual void QCAR_onUpdate(State& state) {
        ...
        for (int i = 0; i<state.getNumTrackables(); ++i) {
            ...
            //
            const Trackable* trackable = state.getTrackable(i);
            if(!trackable) {

                //kasahara
                qcar->markersFound[i].isFound = false;
                continue;
            }
...
bool ofxQCAR::hasFoundMarker() {
    cout << "caution this function is NO MAKING SENSE" << endl;
    return numOfMarkersFound() > 0;
}

bool ofxQCAR::hasFoundMarker(int i)
{
    if(i < numOfMarkersFound()) {
        return markersFound[i].isFound;
    } else {
        return false;
    }
}
